...
def load_blender_data(scene, split="train", white_bg=True, half_res=True):
...
class MLP(nn.Module):
    def __init__(
        ...
        for i in range(net_depth):
            ...
            linear = nn.Linear(dim_in, net_width)
            _xavier_init(linear)
            self.layers.append(nn.Sequential(linear, RePAct()))
        ...
        for i in range(net_depth_condition):
            ...
            linear = nn.Linear(dim_in, net_width_condition)
            _xavier_init(linear)
            self.view_layers.append(nn.Sequential(linear, RePAct()))
...
if __name__ == "__main__":
    ...
    train_images, train_poses, focal, H, W = load_blender_data(
        SCENE_NAME,
        split="train",
        white_bg=WHITE_BKGD,
        half_res=HALF_RES
    ...
    test_images, test_poses, _, _, _ = load_blender_data(
        SCENE_NAME,
        split="test",
        white_bg=WHITE_BKGD,
        half_res=HALF_RES
    ...
    print("Dataset: Blender")
    print(f"Activation: RePAct")
    print(f"Resolution: {H}x{W}")
    print(f"Final PSNR: {final_psnr:.4f}")

    torch.save(model.state_dict(), "mipnerf_blender_repact.pth")