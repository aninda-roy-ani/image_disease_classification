...
def sample_points(rays_o, rays_d, near, far, N):
    t_vals = torch.linspace(near, far, steps=N, device=rays_o.device)
    t_vals = t_vals.view(1, N, 1)
...
def train(model, images, poses, focal, H, W, near, far, device):
    ...
    for epoch in range(100):
        ...
        if epoch < 20:
            ...
            lr = base_lr * (0.995 ** (epoch - 20))
        else:
            lr = base_lr * (0.995 ** 50) * (0.98 ** (epoch - 70))
        ...
        print(f"Epoch {epoch+1} Loss: {total_loss/len(images):.6f} | LR: {lr:.6f}")
...
if __name__ == "__main__":
    ...
    pts, _ = sample_points(dummy_o, dummy_d, near, far, 64)
