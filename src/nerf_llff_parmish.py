...
from src.activations import ParMish
...
def train(model, images, poses, focal, H, W, near, far, epochs=100, lr=5e-4, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        for i in range(images.shape[0]):
            ...
            select = coords[
                torch.randint(0, coords.shape[0], (n_rand,), device=device)
            ]
...
if __name__ == "__main__":
    ...
    images, poses, focal, H, W, near, far = load_llff_data(scene_name, factor=8)
    ...
    model = NerfModel(pos_dim=pos_dim, dir_dim=dir_dim, hidden_dim=128)
    ...
    if final_train_loss is not None:
        ...
        torch.save(model.state_dict(), "nerf_llff_parmish_block123.pth")